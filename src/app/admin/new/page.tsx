...
export default function AdminNewOrEdit() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const t = title.trim();
    const b = body.trim();
    if (t.length < 3) { setErr("Title is too short."); setSaving(false); return; }
    if (b.length < 20) { setErr("Body is too short."); setSaving(false); return; }
    ...
    if (isEdit) {
      ...
      const { error } = await supabase
        .from("entries")
        .insert({ title: t, slug, body: b, published });
      ...
      <main className="min-h-screen grid place-items-center p-6 bg-gray-50">
        <form onSubmit={submit} className="w-full max-w-xl rounded-2xl border bg-white p-6 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link href="/admin" className="text-sm text-teal-700 hover:text-teal-800">Back</Link>
          </div>
